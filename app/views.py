...
def obtener_parrafos_existentes(item_id, tipo_contenido):
    ...
    try:
        if tipo_contenido == 'servicio':
            from .models import ServicioParrafo
            parrafos = ServicioParrafo.objects.filter(servicio_id=item_id).order_by('orden')
        else:
            from .models import ProyectoParrafo
            parrafos = ProyectoParrafo.objects.filter(proyecto_id=item_id).order_by('orden')
